...
import { addTokenCost, initializeTokenCostsTable } from '@/lib/token-costs-db'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Add the token cost record
    const result = await addTokenCost({
      date: body.date,
    ...
    return NextResponse.json(
      { error: 'Failed to add token cost data', details: String(error) },
      { status: 500 }
    )
  }
}
